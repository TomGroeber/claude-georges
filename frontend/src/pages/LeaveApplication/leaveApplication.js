...
const LeaveApplication = () => {
    ...
    const [leaveDateRange, setLeaveDateRange] = useState({
        startDate: new Date(location?.state?.leaveRequest?.startDate),
        endDate: new Date(location?.state?.leaveRequest?.endDate)
    });
    ...
    const handleSubmit = (status, skip = false) => {
        ...
        if (editTime) {
            ...
            startDateTime.setHours(moment(location?.state?.leaveRequest?.startDate).format('HH'), moment(location?.state?.leaveRequest?.startDate).format('mm'), 0, 0);
            endDateTime.setHours(moment(location?.state?.leaveRequest?.endDate).format('HH'), moment(location?.state?.leaveRequest?.endDate).format('mm'), 0, 0);
        }
        ...
        Api.leaveStatusUpdate(location?.state?.leaveRequest?.id, payload).then((res) => {
                ...
                <div className="flex-wrap lg:flex px-4 py-10 rounded-[10px] bg-white gap-4">
                    ...
                    <div className="space-y-6 m-auto lg:m-0 w-full lg:w-[430px]">
                        ...
                        <CommonInput
                            id="fromUser"
                            name="fromUser"
                            value={location?.state?.fromUser?.firstName + ' ' + location?.state?.fromUser?.lastName}
                            type="text"
                        ...
                        <CommonInput
                            id="reason"
                            name="reason"
                            value={location?.state?.leaveRequest?.reason}
                            type="text"
                            label={t('reason')}
                            disabled={true}
                        />
                                                ...
                                                <div className='flex flex-col m-auto lg:m-0 w-full lg:w-[430px]'>
                                                    <div className='flex gap-x-6 md:gap-x-3 mb-3 w-full'>
                                                        <div className='!w-full mb-1 flex items-center gap-x-3'>
                                                            {location?.state &&
                                                                <p className='block text-sm font-medium text-gray-700'>{t('userSelectedSlots')} : {moment(location?.state?.leaveRequest?.startDate).format('HH:mm')} ~ {moment(location?.state?.leaveRequest?.endDate).format('HH:mm')}
                                                                </p>
                                                            }
                                                            <button
                        ...
                        <CommonTextarea
                            ...
                            label={t('comment')}
                            disabled={location?.state?.leaveRequest?.status != 'pending'}
                        />
